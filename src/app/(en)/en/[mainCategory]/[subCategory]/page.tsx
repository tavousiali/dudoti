...
export async function generateStaticParams() {
  ...
  for (const main of mains) {
    ...
    const subs = await prisma.productCategory.findMany({
      where: { ParentId: main.Id, Deleted: false, Lang: 2 },
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const cat = await prisma.productCategory.findFirst({
    where: { urlTitle: subCategory, Deleted: false, Lang: 2 },
  ...
  const canonical = `https://dudoti.com/en/${mainCategory}/${subCategory}/`;
  return { title, description, alternates: { canonical } };
}

export default async function EnSubCategoryPage({ params }: Props) {
  const { mainCategory, subCategory } = await params;
  ...
  const subCat = await prisma.productCategory.findFirst({
    where: { urlTitle: subCategory, ParentId: mainCat.Id, Deleted: false, Lang: 2 },
    select: { Id: true },
  });
  if (!subCat) notFound();
  return <ProductsListPage mainUrlTitle={mainCategory} subCatUrlTitle={subCategory} langId={2} />;
}
