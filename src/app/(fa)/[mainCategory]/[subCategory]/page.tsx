...
export default async function FaSubCategoryPage({ params }: Props) {
  const { mainCategory, subCategory } = await params;
  ...
  const subCat = await prisma.productCategory.findFirst({
    where: { urlTitle: subCategory, ParentId: mainCat.Id, Deleted: false, Lang: 1 },
    select: { Id: true },
  });
  if (!subCat) notFound();
  return <ProductsListPage mainUrlTitle={mainCategory} subCatUrlTitle={subCategory} langId={1} />;
}
