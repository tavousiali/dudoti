  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      const res = await fetch("/api/admin/product-categories", {
        ...
        body: JSON.stringify({ ...data, ParentId: 0, Lang: 1 }),
      });
      ...
      <div style={{
            ...
            <FieldRow label="بارگذاری عکس" helpText="?">
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                <input type="text" readOnly placeholder="No file chosen"
                  ...
                  style={inputStyle} />
                <button type="button" onClick={() => picInputRef.current?.click()} style={chooseFileStyle}>
                  Choose File
                </button>
              </div>
            </FieldRow>
        ...
        <Section>
          <FieldRow label="عنوان SEO">
            <input {...register("SeoTitle")} style={inputStyle} placeholder="انواع خوراک سگ" />
          </FieldRow>
        </Section>
        ...
        {/* خلاصه */}
        <Section>
          <FieldRow label="خلاصه (Lead)" helpText="(Title)">
        ...
        <Section>
          <FieldRow label="بارگذاری ویدیو" helpText="?">
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
              ...
              <button type="button" onClick={() => videoInputRef.current?.click()} style={chooseFileStyle}>
                Choose File
              </button>
            </div>
          </FieldRow>
        ...
        <Section>
          <FieldRow label="بارگذاری پوستر ویدیو" helpText="?">
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
              ...
              <button type="button" onClick={() => videoPicInputRef.current?.click()} style={chooseFileStyle}>
                Choose File
              </button>
            </div>
          </FieldRow>
        ...
        <Section>
          ...
          <div style={{ display: "flex", gap: "12px" }}>
            <button type="submit" disabled={saving} style={{
              ...
              padding: "11px 32px", borderRadius: "7px",
...
function FieldRow({ label, required, helpText, children }: {
  ...
  return (
    <div style={{ display: "flex", alignItems: "flex-start", gap: "16px", direction: "rtl" }}>
      <div style={{
        minWidth: "160px", paddingTop: "8px",
        fontSize: "13px", color: "#555", fontWeight: 500,
        textAlign: "right", display: "flex", alignItems: "center", gap: "4px",
      }}>
        {required && <span style={{ color: "#e74c3c" }}>*</span>}
        {label}
        {helpText && (
          <span style={{
            background: "#eee", color: "#888", borderRadius: "50%",
            width: "16px", height: "16px",
            display: "inline-flex", alignItems: "center", justifyContent: "center",
            fontSize: "10px", cursor: "help", marginRight: "4px",
          }}>{helpText}</span>
        )}
      </div>
...
const chooseFileStyle: React.CSSProperties = {
  ...
  border: "1px solid #ccc", padding: "9px 14px",
