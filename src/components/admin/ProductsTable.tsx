...
interface Cat { Id: number; Title: string; ParentName: string | null; }
...
export default function ProductsTable({
  ...
  initialCatId,
  initialTotal,
  allCats: initialAllCats,
  initialLang = 1,
}: Props) {
  const { lang } = useAdminLang();

  const [products, setProducts] = useState<Product[]>(initialProducts);
  const [allCats, setAllCats] = useState<Cat[]>(initialAllCats);
  ...
  const fetchProducts = useCallback(
    async (langId: number, catId: number | null, sid: string, stitle: string, p: number) => {
      setLoading(true);
      const params = new URLSearchParams({ lang: String(langId), limit: String(LIMIT), page: String(p) });
  ...
  // وقتی زبان عوض شد: دسته‌بندی‌ها + محصولات رو از نو بگیر
  useEffect(() => {
    // skip اولین render (داده server-side داریم)
    ...
    // بارگذاری دسته‌بندی‌های زبان جدید
    fetch(`/api/admin/product-categories?lang=${lang}&parentId=1000`)
      .then(() => { }) // placeholder
      .catch(() => { });

    fetch(`/api/admin/product-categories?lang=${lang}`)
      .then((r) => r.json())
      .then((json) => {
        if (json.success) {
          setAllCats(json.data.filter((c: { ParentId: number }) => c.ParentId !== 0)
            .map((c: { Id: number; Title: string; ParentName: string | null }) => ({
              Id: c.Id, Title: c.Title, ParentName: c.ParentName,
            })));
        }
      });

    fetchProducts(lang, null, "", "", 1);
  ...
  const handleCatChange = (val: string) => {
    ...
    setPage(1);
    fetchProducts(lang, cid, searchId, searchTitle, 1);
    window.history.replaceState(null, "",
      cid ? `/AdminPanel/products?catId=${cid}` : `/AdminPanel/products`);
  };

  const handlePage = (p: number) => {
    setPage(p);
    fetchProducts(lang, selectedCatId, searchId, searchTitle, p);
  };
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "4px" }}>
                    <input type="text" value={searchTitle}
                      onChange={(e) => {
                        ...
                        setPage(1);
                        fetchProducts(lang, selectedCatId, searchId, e.target.value, 1);
                      }}
                      style={filterInput} placeholder="" />
                    <span style={filterIcon}>▼</span>
                  </div>
          ...
          {Array.from({ length: totalPages }, (_, i) => i + 1)
            ...
            .map((n, i) =>
              n === "…"
                ...
                : <button key={n} onClick={() => handlePage(n as number)} style={pgBtn(false, n === page)}>{n}</button>
            )}
...
function ProductRow({ product: p, idx, onDeleted }: {
  product: Product; idx: number; onDeleted: () => void;
}) {
  ...
  return (
    <tr style={{ borderBottom: "1px solid #ececec", background: idx % 2 === 0 ? "#fff" : "#f7f7f7" }}>
      <td style={{ padding: "10px 14px", color: "#888", fontSize: "13px", textAlign: "center", width: "70px" }}>
        {p.Id}
      </td>
      <td style={{ padding: "10px 14px", textAlign: "right" }}>
        <span style={{ fontWeight: 500, color: "#333", fontSize: "13px" }}>{p.Title ?? "—"}</span>
        {p.TitleEn && (
          <div style={{ fontSize: "11px", color: "#aaa", direction: "ltr", marginTop: "2px" }}>{p.TitleEn}</div>
        )}
      </td>
      <td style={{ padding: "6px 14px", textAlign: "center", width: "90px" }}>
        {p.Pic1 ? (
          <Image src={p.Pic1} alt={p.Title ?? ""} width={56} height={56}
            style={{ objectFit: "contain", borderRadius: "4px", border: "1px solid #e8e8e8", background: "#fff", padding: "2px" }} />
        ) : (
          <div style={{ width: "56px", height: "56px", margin: "0 auto", background: "#f0f0f0", borderRadius: "4px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "20px" }}>🛍️</div>
        )}
      </td>
      <td style={{ padding: "10px 14px", textAlign: "center", width: "100px" }}>
        <Link href={`/AdminPanel/products/${p.Id}/edit`}
      ...
      <td style={{ padding: "10px 14px", textAlign: "center", width: "90px" }}>
        <button onClick={handleDelete} disabled={pending}
          style={{ display: "inline-flex", alignItems: "center", gap: "4px", background: pending ? "#f5f5f5" : "#fdecea", color: pending ? "#aaa" : "#e74c3c", border: "none", padding: "5px 12px", borderRadius: "6px", fontSize: "13px", fontWeight: 500, cursor: pending ? "not-allowed" : "pointer", fontFamily: "inherit", whiteSpace: "nowrap" }}>
          {pending ? "..." : "🗑️ حذف"}
        </button>
      </td>
    </tr>
...
const filterInput: React.CSSProperties = {
  ...
  fontSize: "12px", width: "80px", outline: "none", fontFamily: "inherit", background: "#fff",
};
const filterIcon: React.CSSProperties = { fontSize: "9px", color: "#999", cursor: "pointer", flexShrink: 0 };
