...
export default function SocialsTable({ initialSocials }: Props) {
  ...
  const [editValues, setEditValues] = useState<{
    Title: string; Link: string; Priority: string; Icon: string;
  ...
  // وقتی زبان تغییر کرد، داده جدید از API بگیر
  useEffect(() => {
    setLoadingData(true);
    setEditingId(null);
    setSaveError(null);
    ...
    setFilterLink("");
    setFilterPriority("");
    fetch(`/api/admin/socials?lang=${lang}`)
      .then((r) => r.json())
      .then((json) => { if (json.success) setSocials(json.data); })
  ...
  const saveEdit = (id: number) => {
    ...
    startTransition(async () => {
      ...
      if (json.success) {
        setSocials((prev) => prev.map((s) => (s.Id === id ? json.data : s)));
        setEditingId(null);
      } else {
        ...
        <table style={{
          ...
          <thead>
            ...
            <tr style={{ background: "#f2f2f2", borderBottom: "2px solid #ddd" }}>
              ...
              <td style={{ padding: "6px 10px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "4px" }}>
                  <input type="text" value={filterLink}
                        ...
                        <div style={{
                          ...
                          <EditField label="عنوان:">
                            <input value={editValues.Title}
                              ...
                              style={editInp} placeholder="telegram" />
                          </EditField>

                          <EditField label="آیکون:">
                            <select value={editValues.Icon}
                          ...
                          <div style={{ display: "flex", gap: "8px", paddingBottom: "2px" }}>
                            <button onClick={() => saveEdit(s.Id)} disabled={pending}
                              style={{
...
const filterInp: React.CSSProperties = {
  ...
  fontSize: "12px", width: "90px", outline: "none", fontFamily: "inherit", background: "#fff",
};
const filterArrow: React.CSSProperties = { fontSize: "9px", color: "#999", flexShrink: 0 };
const editInp: React.CSSProperties = {
  ...
  fontFamily: "inherit", outline: "none", width: "160px", boxSizing: "border-box",
};
