"use client";

import Button from "@/components/layout/Button";
import PageTitle from "@/components/layout/PageTitle";
import { useLocale } from "@/components/layout/LocaleContext";

const t = {
  fa: {
    title: "تماس با ما",
    contactInfo: "اطلاعات تماس",
    email: "ایمیل",
    namePlaceholder: "نام و نام خانوادگی *",
    phonePlaceholder: "تلفن تماس *",
    messagePlaceholder: "متن پیام *",
    send: "ارسال",
  },
  en: {
    title: "Contact Us",
    contactInfo: "Contact Information",
    email: "Email",
    namePlaceholder: "Full Name *",
    phonePlaceholder: "Phone Number *",
    messagePlaceholder: "Message *",
    send: "Send",
  },
  fr: {
    title: "Contactez-nous",
    contactInfo: "Informations de contact",
    email: "E-mail",
    namePlaceholder: "Nom complet *",
    phonePlaceholder: "Numéro de téléphone *",
    messagePlaceholder: "Message *",
    send: "Envoyer",
  },
} as const;

export default function ContactPageContent() {
  const { locale, dir } = useLocale();
  const tr = t[locale];

  return (
    <main className="min-h-screen bg-white" dir={dir}>
      <div className="mx-auto max-w-[1140px] px-4 py-10 md:py-16">
        <div className="pt-44">
          <div className="contact-con rounded-3xl bg-[#f9e0a4] px-6 py-10 sm:px-10 sm:py-12 md:px-14">

            <PageTitle title={tr.title} as="h1" className="mb-8" />

            <div className="flex flex-col gap-10 md:flex-row md:items-start md:gap-16">

              {/* Contact info */}
              <div className="flex shrink-0 flex-col items-start gap-4 md:w-64 lg:w-72">
                <div className="mt-2" style={{ textAlign: dir === "ltr" ? "left" : "right" }}>
                  <p className="mb-2 text-sm font-semibold text-gray-700">
                    {tr.contactInfo}
                  </p>
                  <p className="text-sm text-gray-700">
                    {tr.email}:{" "}
                    <a
                      href="mailto:dudoticompany@gmail.com"
                      className="font-medium text-gray-800 transition-opacity hover:opacity-70"
                      dir="ltr"
                    >
                      dudoticompany@gmail.com
                    </a>
                  </p>
                </div>
              </div>

              {/* Form */}
              <div className="flex flex-1 flex-col gap-4">
                <input
                  type="text"
                  placeholder={tr.namePlaceholder}
                  className="w-full rounded-full bg-white px-6 py-3 text-sm text-gray-700 outline-none placeholder:text-gray-400 focus:ring-2 focus:ring-[#ff2f2f]/30"
                  style={{ textAlign: dir === "ltr" ? "left" : "right" }}
                />
                <input
                  type="tel"
                  placeholder={tr.phonePlaceholder}
                  className="w-full rounded-full bg-white px-6 py-3 text-sm text-gray-700 outline-none placeholder:text-gray-400 focus:ring-2 focus:ring-[#ff2f2f]/30"
                  style={{ textAlign: dir === "ltr" ? "left" : "right" }}
                />
                <textarea
                  placeholder={tr.messagePlaceholder}
                  rows={5}
                  className="w-full resize-none rounded-2xl bg-white px-6 py-4 text-sm text-gray-700 outline-none placeholder:text-gray-400 focus:ring-2 focus:ring-[#ff2f2f]/30"
                  style={{ textAlign: dir === "ltr" ? "left" : "right" }}
                />
                <div className={`flex ${dir === "ltr" ? "justify-start" : "justify-end"}`}>
                  <Button text={tr.send} />
                </div>
              </div>

            </div>
          </div>
        </div>
      </div>
    </main>
  );
}
